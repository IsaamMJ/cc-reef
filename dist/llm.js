...
export class LLMError extends Error {
    status;
    provider;
    constructor(msg, status, provider) {
        super(msg);
        this.status = status;
...
function pickProvider(explicit) {
    const order = explicit
        ? [explicit]
        : (process.env.REEF_LLM_PROVIDER ? [process.env.REEF_LLM_PROVIDER] : ['xai', 'nim', 'openai']);
    for (const p of order) {
        if (p === 'xai' && process.env.XAI_API_KEY)
            return 'xai';
        if (p === 'nim' && process.env.NIM_API_KEY)
            return 'nim';
...
export async function chat(messages, opts = {}) {
    ...
    if (!provider)
        throw new LLMError('No LLM provider configured. Set XAI_API_KEY, NIM_API_KEY, or OPENAI_API_KEY.');
    ...
    try {
        ...
        throw new LLMError(`${provider} fetch failed: ${e.message}`, undefined, provider);
    }
    clearTimeout(tid);
    if (!res.ok) {
...
export async function* chatStream(messages, opts = {}) {
    ...
    try {
        ...
        clearTimeout(tid);
        yield { type: 'error', error: `${provider} fetch failed: ${e.message}` };
        return;
    }
    if (!res.ok || !res.body) {
        clearTimeout(tid);
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (ids.length > 0) {
                ...
                if (!seen.has(pricing.defaultModel))
                    merged.unshift({ id: pricing.defaultModel, provider: p });
                return merged;
            }
        }
    }
    catch (e) {
        log.warn('llm: model discovery failed', { provider: p, err: e.message });
    }
    // Fallback to hardcoded list.
    if (known.length === 0)
        return [{ id: pricing.defaultModel, provider: p }];
    return known;
}
...
function recordCall(rec) {
    try {
      ...
      project TEXT
        ...
        db.prepare(`INSERT INTO llm_calls (started_at, provider, model, purpose, input_tokens, output_tokens, cost_usd, "group", project)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(rec.startedAt, rec.provider, rec.model, rec.purpose, rec.inputTokens, rec.outputTokens, rec.costUsd, rec.group, rec.project);
    }
    catch (e) {
        log.warn('llm: could not record call', { err: e.message });
    }
}
export function getSpendSummary(days = 30) {
    const db = getDb();
