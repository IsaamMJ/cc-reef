...
import { loadConfig, saveConfig, addGroup, linkProject, getGroupForProject, } from './groups.js';
...
function buildStopwords() {
    const out = new Set(GENERIC_STOPWORDS);
    try {
            ...
            if (Array.isArray(raw.stopwords)) {
                for (const w of raw.stopwords) {
                    if (typeof w === 'string' && w.trim())
                        out.add(w.trim().toLowerCase());
                }
            }
        }
    }
    catch (e) {
        log.warn('autogroup: could not load custom stopwords', { err: e.message });
    }
    return out;
}
 ...
 */
function clusterFolders(folders) {
    const PREFIX_LEN = 4;
...
export function runAutoGroup(options = {}) {
    const allProjects = listProjectFolders();
    const cfg = loadConfig();
    const alreadyGrouped = [];
    const unassigned = [];
    for (const p of allProjects) {
        if (getGroupForProject(cfg, p))
    ...
    for (const [key, members] of clusters) {
        ...
        if (!options.dryRun) {
            ...
            for (const folder of members)
                linkProject(cfg, folder, groupName);
        }
