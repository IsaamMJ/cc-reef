...
function collectJsonl(dir) {
    ...
    try {
        ...
        log.warn('readdir failed', { dir, err: e.message });
        return out;
    }
...
function aggregateFromEvent(agg, event) {
    const ts = typeof event.timestamp === 'string' ? event.timestamp : null;
    if (ts) {
        if (!agg.started_at || ts < agg.started_at)
            agg.started_at = ts;
        if (!agg.ended_at || ts > agg.ended_at)
            agg.ended_at = ts;
    }
    ...
    if (model && !agg.primary_model)
        agg.primary_model = model;
...
async function scanFile(filePath, project, fileMtime) {
...
export async function scan(options = {}) {
    ...
    const getExisting = db.prepare('SELECT file_mtime FROM sessions WHERE session_id = ?');
  ...
  `);
    ...
    const insertToolCall = db.prepare('INSERT INTO tool_calls (session_id, project, tool_name, model, ts) VALUES (?, ?, ?, ?, ?)');
    for (const project of projectDirs) {
        ...
        for (const file of files) {
            ...
            if (!options.force) {
                const existing = getExisting.get(sessionId);
            ...
            db.exec('BEGIN');
            try {
                upsertSession.run(agg.session_id, agg.project, agg.source_file, agg.file_mtime, agg.started_at, agg.ended_at, agg.turn_count, agg.tool_call_count, agg.total_input_tokens, agg.total_output_tokens, agg.total_cache_read_tokens, agg.total_cache_creation_tokens, agg.primary_model, agg.parse_errors, scannedAt);
                ...
                for (const tc of agg.tool_events) {
                    insertToolCall.run(agg.session_id, agg.project, tc.tool, tc.model, tc.ts);
                }
                db.exec('COMMIT');
            }
            catch (e) {
