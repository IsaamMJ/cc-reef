...
const TOOLS = [
    {
        name: 'reef_status',
        description: 'Report reef health: whether hooks are installed, DB freshness, ' +
    ...
    {
        name: 'reef_report',
        description: 'Generate a markdown activity report grouped by company/product. ' +
    ...
    {
        name: 'reef_resume',
        description: 'Get a concise "where did I leave off" card for a given project ' +
    ...
    },
    {
        ...
        description: 'List every Claude Code project folder under ~/.claude/projects/ along ' +
            'with its assigned group (or "(ungrouped)").',
        inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    },
    {
        name: 'reef_list_groups',
        description: 'List all groups (companies/products) with their members and any ' +
            'associated company name.',
        inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    },
    {
        name: 'reef_create_group',
        description: 'Create a new group. Use this to represent a company or product ' +
    ...
    },
    {
        ...
        description: 'Assign a Claude Code project folder to a group. The project is ' +
    ...
    {
        name: 'reef_autogroup',
        description: 'Automatically group unassigned project folders by name similarity. ' +
    ...
    },
    {
        ...
        description: 'Merge source group into target group: all source folders move to ' +
    ...
    try {
        switch (name) {
            ...
            case 'reef_create_group': {
                ...
                const cfg = loadConfig();
                if (cfg.groups[n])
                    return errResponse(`Group "${n}" already exists`);
            ...
            case 'reef_assign_group': {
                ...
                if (typeof p !== 'string' || !p.trim())
                    return errResponse('project is required');
                if (typeof g !== 'string' || !g.trim())
                    return errResponse('group is required');
                const cfg = loadConfig();
                if (!cfg.groups[g])
                    return errResponse(`Group "${g}" does not exist. Create it first with reef_create_group.`);
                linkProject(cfg, p, g);
                saveConfig(cfg);
            ...
            case 'reef_resume': {
                ...
                const db = getDb();
                const row = db
                    .prepare(`SELECT session_id, ended_at, turn_count, tool_call_count,
            ...
            case 'reef_rename_group': {
                ...
                if (typeof o !== 'string' || !o.trim())
                    return errResponse('oldName is required');
                if (typeof n !== 'string' || !n.trim())
                    return errResponse('newName is required');
                const cfg = loadConfig();
                renameGroup(cfg, o, n);
                saveConfig(cfg);
            ...
            case 'reef_merge_groups': {
                ...
                if (typeof s !== 'string' || !s.trim())
                    return errResponse('source is required');
                if (typeof t !== 'string' || !t.trim())
                    return errResponse('target is required');
                const cfg = loadConfig();
                mergeGroups(cfg, s, t);
                saveConfig(cfg);
            ...
            case 'reef_set_company': {
                ...
                if (typeof g !== 'string' || !g.trim())
                    return errResponse('group is required');
                if (typeof c !== 'string')
                    return errResponse('company must be a string (pass "" to clear)');
                const cfg = loadConfig();
...
export async function runMcpServer() {
    const server = new Server({ name: 'reef', version: '0.0.1' }, { capabilities: { tools: {} } });
