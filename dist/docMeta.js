...
import { ALL_TYPES, TIER_FOR_TYPE } from './docTaxonomy.js';
import { parseFrontmatter } from './frontmatter.js';
import { log } from './log.js';
function metaPath(group) {
    return join(REEF_KNOWLEDGE, group.toLowerCase(), 'doc-meta.json');
}
export function readDocMeta(group) {
...
function writeDocMeta(group, entries) {
    const p = metaPath(group);
    const dir = dirname(p);
    if (!existsSync(dir))
 ...
 */
export async function enrichDoc(group, absPath, mtime) {
    if (!llmAvailable())
        return null;
    ...
    try {
        ...
        log.warn('docMeta: llm classify failed', { absPath, err: e.message });
        return null;
    }
    const type = (parsed.type && parsed.type in TIER_FOR_TYPE ? parsed.type : 'notes');
    const entry = {
        mtime,
        type,
 ...
 */
export async function enrichDocs(group, docs, concurrency = 3) {
    if (!llmAvailable())
        ...
        workers.push((async () => {
            while (queue.length) {
                ...
                if (!next)
                    return;
                try {
                    await enrichDoc(group, next.absolutePath, next.mtime);
                }
                catch (e) {
                    log.warn('docMeta: enrich error', { err: e.message });
                }
            }
        })());
    }
