...
function tryRegisterMcp(cliPath) {
    ...
    if (which.status !== 0) {
        return {
            status: 'failed',
            detail: '`claude` CLI not found on PATH. Install Claude Code, or run this manually:\n' +
    ...
    const add = spawnSync('claude', ['mcp', 'add', '--scope', 'user', 'reef', '--', 'node', cliPath, 'mcp'], { encoding: 'utf8' });
    if (add.status !== 0) {
        return {
            status: 'failed',
            detail: (add.stderr || add.stdout || 'unknown error').trim() ||
...
export async function runInit(opts = {}) {
    ...
    if (!opts.skipHooks) {
        ...
        if (h.backupPath)
            console.log(`      backup:   ${h.backupPath}`);
    }
    if (!opts.skipMcp) {
    ...
    if (!opts.skipScan) {
        ...
        console.log(`      ${s.sessionsUpserted} upserted, ${s.filesSkipped} already fresh (${s.durationMs}ms)`);
        closeDb();
    }
    if (!opts.skipAutogroup) {
        ...
        console.log(`      ${g.created.length} group(s) auto-created, ${g.alreadyGrouped.length} already grouped, ${g.skippedSingletons.length} singleton(s) left`);
