...
import { scanGroupRepoDocs, readDecisions, resolveProjectPath } from './docs.js';
...
function summariseDecisions(group, limit = 10) {
    try {
        const all = readDecisions(group);
        if (!all.length)
            return '_(no decisions logged yet)_';
        ...
        log.warn('chatContext: decisions read failed', { err: e.message });
 ...
 */
export function buildGroupContextWithOverride(group, override) {
 ...
 */
export function buildArchitectModePrompt(group) {
    const base = buildGroupContext(group);
...
export function buildAuditModePrompt(group, docName) {
    const base = buildGroupContext(group);
    const refStart = base.systemPrompt.indexOf('# Documentation index');
    const refTail = refStart >= 0 ? base.systemPrompt.slice(refStart) : '';
 ...
 */
export function resolveDocRef(ctx, ref) {
    const r = ref.trim();
    if (!r)
        return null;
...
export function readDocBody(d) {
    if (!existsSync(d.absolutePath))
        return `_(doc no longer exists: ${d.relPath})_`;
    try {
        ...
        return `_(read failed: ${e.message})_`;
    }
}
/**
 ...
 */
export function expandMentions(ctx, userMessage, maxDocs = 5) {
    const out = [];
    const seen = new Set();
    ...
    while ((m = mentionRe.exec(userMessage)) !== null && out.length < maxDocs) {
        ...
        if (doc)
            out.push({ ref, doc, body: readDocBody(doc) });
    }
    return out;
}
