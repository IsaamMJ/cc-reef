...
import { extractClaims as extractInlineClaims } from './drift.js';
import { log } from './log.js';
function cachePath(group) {
...
function writeCache(group, cache) {
    const p = cachePath(group);
    const dir = dirname(p);
    if (!existsSync(dir))
...
export async function extractLlmClaims(group, docAbsPath, docRelPath, content) {
    if (!llmAvailable())
        return [];
    let mtime = 0;
    try {
        mtime = statSync(docAbsPath).mtimeMs;
    }
    catch { /* ignore */ }
    ...
    try {
        ...
        log.warn('claimsExtract: LLM call failed', { group, doc: docRelPath, err: e.message });
        return cached?.claims ?? [];
    }
    cache[docAbsPath] = {
        mtime,
    ...
    try {
        writeCache(group, cache);
    }
    catch (e) {
        log.warn('claimsExtract: cache write failed', { err: e.message });
    }
    return claims;
}
export function combineClaims(inline, llm) {
...
//# sourceMappingURL=claimsExtract.js.map