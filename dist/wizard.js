...
import { loadConfig, saveConfig, listGroupNames, addGroup, linkProject, unlinkProject, removeGroup, getUnassignedProjects, getGroupForProject, suggestGroupKey, UNGROUPED, } from './groups.js';
import { isAbortError } from './errors.js';
function groupChoices(cfg) {
 ...
 */
export async function promptForGroupAssignment(project) {
    const cfg = loadConfig();
    const suggestion = suggestGroupKey(project);
    const existingNames = listGroupNames(cfg);
    const defaultValue = existingNames.find((n) => n.toLowerCase() === suggestion.toLowerCase());
    ...
    if (choice === 'new') {
        const name = await input({
            ...
            default: suggestion,
            validate: (v) => v.trim().length > 0 ? true : 'Group name cannot be empty',
        });
 ...
 */
export async function promptForAllUnassigned(allProjects) {
    const cfg = loadConfig();
...
function printGroupings(cfg, allProjects) {
...
async function wizardLink(allProjects) {
    const cfg = loadConfig();
    const names = listGroupNames(cfg);
    if (names.length === 0) {
...
async function wizardUnlink() {
    const cfg = loadConfig();
    const assigned = [];
    for (const [groupName, def] of Object.entries(cfg.groups)) {
        for (const p of def.projects)
            assigned.push({ project: p, group: groupName });
    }
...
export async function runGroupsWizard(allProjects) {
    while (true) {
        ...
        try {
            if (action === 'exit')
                return;
            if (action === 'view')
                printGroupings(loadConfig(), allProjects);
