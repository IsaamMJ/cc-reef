...
import { resolveProjectPath, readDecisions } from './docs.js';
...
export function extractToolCalls(text) {
    ...
    while ((m = HEADER_RE.exec(text)) !== null) {
        ...
        if (!name) {
            ...
            if (!bare || !KNOWN_TOOLS.has(bare))
                continue;
            name = bare;
        }
        const headerStart = m.index;
        if (seenStarts.has(headerStart))
            continue;
    ...
    const dedup = [];
    for (const c of out) {
        if (dedup.some((d) => Math.abs(d.startIdx - c.startIdx) < 10 || (d.startIdx <= c.startIdx && d.endIdx >= c.endIdx)))
            continue;
        dedup.push(c);
    }
    return dedup;
...
function sandboxRoots(group) {
    ...
    for (const dp of def.docPaths ?? []) {
        if (existsSync(dp))
            roots.add(resolve(dp));
    }
    return [...roots];
}
function withinSandbox(roots, abs) {
    const a = resolve(abs);
...
function runReadDoc(group, args) {
    const ref = String(args.name ?? args.path ?? args.id ?? '').trim();
    if (!ref)
        return { ok: false, summary: 'read_doc: missing "name"', body: 'Error: provide {"name": "<doc-stem-or-path>"}.' };
...
function runListDecisions(group, args) {
    const limit = Math.max(1, Math.min(50, Number(args.limit) || 20));
    const all = readDecisions(group);
    const recent = all.slice(-limit).reverse();
    if (recent.length === 0)
        return { ok: true, summary: 'list_decisions · 0 decisions', body: '_(no decisions logged for this project yet)_' };
...
function runListFiles(group, args) {
    const roots = sandboxRoots(group);
    if (roots.length === 0)
        return { ok: false, summary: 'list_files: no project roots', body: 'No project repos configured for this group.' };
    ...
    if (!subPath) {
        ...
        for (const r of roots) {
            ...
            if (withinSandbox(roots, p) && existsSync(p) && statSync(p).isDirectory()) {
                target = p;
                break;
            }
        }
        if (!target)
            return { ok: false, summary: `list_files: not found "${subPath}"`, body: `Directory not found inside the project sandbox: ${subPath}` };
    ...
    function walk(root, cur, acc, cap) {
        if (acc.length >= cap)
            return;
        let entries = [];
        try {
...
function runReadFile(group, args) {
    const roots = sandboxRoots(group);
    const p = String(args.path ?? '').trim();
    if (!p)
        return { ok: false, summary: 'read_file: missing "path"', body: 'Error: provide {"path": "<relative-or-absolute-path>"}.' };
    let abs = null;
    if (isAbsolute(p) && withinSandbox(roots, p) && existsSync(p))
        abs = p;
    else {
        for (const r of roots) {
            const cand = resolve(r, p);
            if (withinSandbox(roots, cand) && existsSync(cand) && statSync(cand).isFile()) {
                abs = cand;
                break;
            }
        }
    ...
    if (!abs)
        return { ok: false, summary: `read_file: not found "${p}"`, body: `File not found in sandbox: ${p}` };
    const name = abs.split(/[\\/]/).pop() ?? '';
    if (isBinary(name))
        return { ok: false, summary: `read_file: binary "${name}"`, body: `Binary files not supported: ${name}` };
    ...
    try {
        ...
        return { ok: false, summary: `read_file: ${e.message}`, body: `Read failed: ${e.message}` };
    }
...
function runGrepCode(group, args) {
    const roots = sandboxRoots(group);
    if (roots.length === 0)
        return { ok: false, summary: 'grep_code: no roots', body: 'No project repos.' };
    const patternStr = String(args.pattern ?? '').trim();
    if (!patternStr)
        return { ok: false, summary: 'grep_code: missing "pattern"', body: 'Error: provide {"pattern": "<regex>"}.' };
    let re;
    try {
        re = new RegExp(patternStr, args.caseSensitive === true ? '' : 'i');
    }
    catch (e) {
        return { ok: false, summary: `grep_code: bad pattern`, body: `Invalid regex: ${e.message}` };
    }
    ...
    if (targetRoots.length === 0)
        return { ok: false, summary: `grep_code: scope not found "${scope}"`, body: `Scope not found: ${scope}` };
    const SCAN_LIMIT = 2000;
    for (const r of targetRoots) {
        walk(r, r);
        if (matches.length >= max || scanned >= SCAN_LIMIT)
            break;
    }
    function walk(root, cur) {
        if (matches.length >= max || scanned >= SCAN_LIMIT)
            return;
        let entries = [];
        try {
    ...
    if (matches.length === 0)
        return { ok: true, summary: `🔎 grep_code · 0 matches`, body: `No matches for /${patternStr}/${scope ? ' in ' + scope : ''}.` };
    return { ok: true, summary: `🔎 grep_code · ${matches.length} match${matches.length === 1 ? '' : 'es'}`, body: matches.join('\n') };
}
function asPriority(v) {
    return (['high', 'med', 'low'].includes(String(v)) ? String(v) : 'med');
}
function asStatus(v) {
    return (['todo', 'in_progress', 'done'].includes(String(v)) ? String(v) : 'todo');
}
function runProposeTask(group, args) {
    const title = String(args.title ?? '').trim();
    if (!title)
        return { ok: false, summary: 'propose_task: missing "title"', body: 'Error: provide {"title": "...", "description"?, "priority"?, "docRef"?, "evidence"?}.' };
...
function runUpdateTask(group, args) {
    const id = String(args.id ?? '').trim();
    if (!id)
        return { ok: false, summary: 'update_task: missing "id"', body: 'Error: provide {"id": "t-...", "status"?, "priority"?, "title"?, "description"?}.' };
    const patch = {};
    ...
    if (args.priority !== undefined)
        patch.priority = asPriority(args.priority);
    if (args.status !== undefined)
        patch.status = asStatus(args.status);
    if (args.docRef !== undefined)
        patch.docRef = String(args.docRef);
    if (args.evidence !== undefined)
        patch.evidence = String(args.evidence);
    const t = updateTask(group, id, patch);
    if (!t)
        return { ok: false, summary: `update_task: not found "${id}"`, body: `No task with id ${id}.` };
    return { ok: true, summary: `✏️ update_task · ${t.id} → ${t.status}`, body: `Updated ${t.id}: status=${t.status}, priority=${t.priority}.` };
}
function runListTasks(group, args) {
    const status = args.status ? String(args.status) : '';
    let items = listTasks(group);
    if (status)
        items = items.filter((t) => t.status === status);
    if (items.length === 0)
        return { ok: true, summary: `📋 list_tasks · 0`, body: '_(no tasks)_' };
    const lines = items.slice(0, 50).map((t) => `- ${t.id} [${t.priority}] [${t.status}] ${t.title}${t.docRef ? ` (about ${t.docRef})` : ''}`);
    return { ok: true, summary: `📋 list_tasks · ${items.length}`, body: lines.join('\n') };
}
// ---------------- Apply-doc-change (orchestrated; reliable for big docs) ----------------
async function runApplyDocChange(group, args) {
    ...
    if (!ref)
        return { ok: false, summary: 'apply_doc_change: missing "docRef"', body: 'Provide {"docRef": "<doc>", "change": "<what to change>", "reason"?: "<why>"}.' };
    if (!change)
        return { ok: false, summary: 'apply_doc_change: missing "change"', body: 'Describe the change in plain English; the runner will rewrite the doc.' };
    const ctx = buildGroupContext(group);
    const doc = resolveDocRef(ctx, ref);
    if (!doc)
        return { ok: false, summary: `apply_doc_change: not found "${ref}"`, body: `Doc "${ref}" not found.` };
    ...
    try {
        ...
        return { ok: false, summary: 'apply_doc_change: LLM call failed', body: `LLM error: ${e.message}` };
    }
    ...
    if (objStart === -1)
        return { ok: false, summary: 'apply_doc_change: no JSON', body: 'LLM did not return JSON.' };
    let parsed = {};
    try {
        parsed = JSON.parse(txt.slice(objStart));
    }
    catch (e) {
        return { ok: false, summary: 'apply_doc_change: bad JSON', body: `JSON parse failed: ${e.message}` };
    }
    const newBody = (parsed.newBody ?? '').trim();
    if (!newBody)
        return { ok: false, summary: 'apply_doc_change: empty body', body: 'LLM returned an empty replacement body.' };
    ...
    try {
        ...
        return { ok: false, summary: 'apply_doc_change: write failed', body: `Write error: ${e.message}` };
    }
    return {
        ok: true,
...
function runUpdateDoc(group, args) {
    ...
    const newBody = String(args.newBody ?? args.body ?? args.content ?? args.markdown ?? args.text ?? args.contents ?? '').trim();
    const reason = String(args.reason ?? args.why ?? '').trim();
    if (!ref)
        return { ok: false, summary: 'update_doc: missing "docRef"', body: 'Provide {"docRef": "<doc-stem-or-path>", "newBody": "<full markdown>", "reason": "..."}.' };
    if (!newBody)
        return { ok: false, summary: 'update_doc: missing "newBody"', body: 'Provide the FULL replacement markdown for the doc, not a diff.' };
    const ctx = buildGroupContext(group);
    const doc = resolveDocRef(ctx, ref);
    if (!doc)
        return { ok: false, summary: `update_doc: not found "${ref}"`, body: `Doc "${ref}" not found in this project.` };
    ...
    try {
        ...
        return { ok: false, summary: `update_doc: write failed`, body: `Write failed: ${e.message}` };
    }
    return {
        ok: true,
...
function pickGitRoot(group, args) {
    const roots = sandboxRoots(group);
    const p = String(args.path ?? '').trim();
    // Pick the first sandbox root that's inside a git repo, OR resolve `path` and walk up from it.
    let target = null;
    if (p) {
        if (isAbsolute(p) && withinSandbox(roots, p))
            target = p;
        else {
            for (const r of roots) {
                const cand = resolve(r, p);
                if (withinSandbox(roots, cand) && existsSync(cand)) {
                    target = cand;
                    break;
                }
            }
...
function runGitLog(group, args) {
    const ctx = pickGitRoot(group, args);
    if (!ctx)
        return { ok: false, summary: 'git_log: no repo', body: 'No git repo found inside the project sandbox.' };
    ...
    if (ctx.relPath) {
        cmd.push('--');
        cmd.push(ctx.relPath);
    }
    let out = '';
    try {
        out = gitRun(ctx.root, cmd);
    }
    catch (e) {
        return { ok: false, summary: 'git_log: failed', body: `git error: ${e.message}` };
    }
    if (!out.trim())
        return { ok: true, summary: `🪵 git_log · 0 commits`, body: '_(no commits match)_' };
    const lines = out.split('\n').filter(Boolean);
    return { ok: true, summary: `🪵 git_log · ${lines.length} commits${ctx.relPath ? ' · ' + ctx.relPath : ''}`, body: lines.join('\n') };
}
function runGitShow(group, args) {
    const ctx = pickGitRoot(group, args);
    if (!ctx)
        return { ok: false, summary: 'git_show: no repo', body: 'No git repo found.' };
    const ref = String(args.ref ?? args.hash ?? '').trim();
    if (!ref)
        return { ok: false, summary: 'git_show: missing "ref"', body: 'Provide {"ref": "<commit-hash-or-ref>"}.' };
    ...
    try {
        ...
        return { ok: false, summary: 'git_show: failed', body: `git error: ${e.message}` };
    }
...
function runGitDiff(group, args) {
    const ctx = pickGitRoot(group, args);
    if (!ctx)
        return { ok: false, summary: 'git_diff: no repo', body: 'No git repo found.' };
    ...
    try {
        ...
        return { ok: false, summary: 'git_diff: failed', body: `git error: ${e.message}` };
    }
    const cap = Math.min(8000, Number(args.maxChars) || 5000);
    const truncated = out.length > cap;
    if (truncated)
...
function findLivingTruthFiles(roots) {
    const out = [];
    for (const r of roots) {
        walk(r, r, 0);
    }
    function walk(root, cur, depth) {
        ...
        for (const name of entries) {
            ...
            if (st.isDirectory())
                ...
                const rel = relative(root, full).split(sep).join('/');
                if (LIVING_TRUTH_NAMES.includes(name))
                    out.push(full);
                else if (LIVING_TRUTH_PATTERNS.some((re) => re.test(rel) || re.test(name)))
                    out.push(full);
            }
        }
    }
...
function runReadLivingTruth(group, args) {
    const roots = sandboxRoots(group);
    const files = findLivingTruthFiles(roots);
    if (files.length === 0)
        return { ok: true, summary: '📜 living_truth · 0 files', body: '_(no CLAUDE.md / AGENTS.md / drift log / ADR files found)_' };
...
// ---------------- Dispatcher ----------------
export async function runTool(group, name, args) {
    try {
        switch (name) {
        ...
        log.warn('chatTools: tool threw', { name, err: e.message });
        return { ok: false, summary: `${name}: error`, body: `Tool execution error: ${e.message}` };
    }
}
/**
