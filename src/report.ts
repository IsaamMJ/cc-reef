...
function sinceIso(opts: ReportOptions): string {
...
export function generateReport(opts: ReportOptions = {}): string {
  ...
  const sessions = db
    .prepare(
      `SELECT session_id, project, started_at, ended_at, turn_count, tool_call_count,
  ...
  for (const s of sessions) {
    ...
    if (!row.projects.includes(s.project)) row.projects.push(s.project);
    row.sessions++;
    ...
    if (!row.primaryModel && s.primary_model) row.primaryModel = s.primary_model;
  }
  ...
  for (const row of sortedGroups) {
    ...
    lines.push('');
    lines.push(
      `- Sessions: **${row.sessions}** · Turns: **${row.turns.toLocaleString()}** · ` +
        `Tool calls: **${row.toolCalls.toLocaleString()}** · ` +
        `Tokens: **${fmtTokens(row.inputTokens + row.outputTokens)}**`,
    );
    if (row.primaryModel) lines.push(`- Primary model: \`${row.primaryModel}\``);
  ...
  lines.push(
    `- Bash / native ratio: **${Number.isFinite(ratio) ? ratio.toFixed(2) : '∞'}** ` +
  ...
  if (ratio >= 1) {
    insights.push(
      `Bash is used more than native search/read tools — the PreToolUse hook should help reduce this over time.`,
    );
  }
  const opusSessions = modelRows.find((r) =>
    r.primary_model.includes('opus'),
  )?.c ?? 0;
  const haikuSessions = modelRows.find((r) =>
    r.primary_model.includes('haiku'),
  )?.c ?? 0;
  if (opusSessions > haikuSessions * 2 && opusSessions > 10) {
    insights.push(
      `Opus is dominant (${opusSessions} vs ${haikuSessions} Haiku sessions). Consider Haiku for short, tool-light tasks.`,
    );
  }
  const ungrouped = groups.get(UNGROUPED);
  if (ungrouped && ungrouped.projects.length > 0) {
    insights.push(
      `${ungrouped.projects.length} project folder(s) are ungrouped — run \`reef groups\` to label them.`,
    );
  }
  if (insights.length === 0) insights.push(`Nothing surfacing — your patterns look clean this window.`);
...
export function writeReport(opts: ReportOptions = {}): {
  content: string;
  outFile: string | null;
} {
  const content = generateReport(opts);
  if (!opts.outFile) return { content, outFile: null };
