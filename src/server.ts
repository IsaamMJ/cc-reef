...
import { scanGroupRepoDocs, readDecisions, appendDecision, type DocFile } from './docs.js';
...
import {
  listSessions as listChatSessions, getSession as getChatSession,
  createSession as createChatSession, deleteSession as deleteChatSession,
  appendMessage as appendChatMessage, touchSession as touchChatSession,
  autoTitleIfDefault as autoTitleChatIfDefault,
  type ChatTurn,
} from './chat.js';
...
async function apiOverview(days: number): Promise<unknown> {
  ...
  const sessions = db
    ...
    .all(since) as Array<{
      session_id: string; project: string; ended_at: string | null;
      turn_count: number; tool_call_count: number;
      total_input_tokens: number; total_output_tokens: number;
      primary_model: string | null; summary: string | null;
    }>;
  ...
  for (const s of sessions) {
    ...
    if (d && d in heatmap) heatmap[d] = (heatmap[d] ?? 0) + 1;
  }
  ...
  const momentumDays: string[] = [];
  for (let i = 13; i >= 0; i--) {
    momentumDays.push(new Date(Date.now() - i * 86400000).toISOString().slice(0, 10));
  }
  const momentum = new Map<string, Record<string, number>>();
  for (const s of sessions) {
    const g = getGroupForProject(cfg, s.project) ?? UNGROUPED;
    const d = s.ended_at?.slice(0, 10);
    if (!d || !momentumDays.includes(d)) continue;
  ...
  const recent = db.prepare(
    `SELECT session_id, project, ended_at, turn_count, tool_call_count, primary_model, summary
     FROM sessions WHERE ended_at IS NOT NULL ORDER BY ended_at DESC LIMIT 20`,
  ).all() as Array<{
    session_id: string; project: string; ended_at: string | null;
    turn_count: number; tool_call_count: number; primary_model: string | null;
    summary: string | null;
  }>;
  ...
  if (nimAvailable()) {
    try { aiLine = await aiOverviewLine(days); } catch { aiLine = null; }
  }
...
function apiProjectsList(): unknown {
  const cfg = loadConfig();
  const db = getDb();
  const sessionsByProject = db.prepare(
    `SELECT project, COUNT(*) c, MAX(ended_at) last FROM sessions GROUP BY project`,
  ).all() as Array<{ project: string; c: number; last: string | null }>;
  closeDb();
  ...
  const rows = Object.keys(cfg.groups).sort().map((key) => {
    ...
    const lastList = def.projects.map((p) => projectMap.get(p)?.last).filter(Boolean) as string[];
  ...
  const companies = [...byCompany.entries()]
    ...
    .sort((a, b) => {
      if (a.company === PERSONAL) return 1;
      if (b.company === PERSONAL) return -1;
...
function apiProjectDetail(group: string): unknown {
  const cfg = loadConfig();
  const def = cfg.groups[group];
  if (!def) return { error: 'group not found' };
  const trust = getGroupTrust(cfg, group);
  if (trust === 'deny') return { error: `trust tier is "deny"` };
  ...
  const sessions = def.projects.length > 0
    ? db.prepare(
        `SELECT session_id, project, ended_at, turn_count, tool_call_count, primary_model,
                total_input_tokens, total_output_tokens
         FROM sessions WHERE project IN (${placeholders}) AND ended_at IS NOT NULL AND ended_at >= ?
         ORDER BY ended_at DESC`,
      ).all(...def.projects, since) as Array<{
          session_id: string; project: string; ended_at: string | null;
          turn_count: number; tool_call_count: number; primary_model: string | null;
          total_input_tokens: number; total_output_tokens: number;
        }>
  ...
  if (sessionIds.length > 0) {
    ...
    topTools = (db.prepare(
      `SELECT tool_name, COUNT(*) c FROM tool_calls WHERE session_id IN (${ph})
       GROUP BY tool_name ORDER BY c DESC LIMIT 10`,
    ).all(...sessionIds) as Array<{ tool_name: string; c: number }>)
      .map((r) => ({ tool: r.tool_name, count: r.c }));
  }

  const modelCount = new Map<string, number>();
  for (const s of sessions) {
    if (s.primary_model) modelCount.set(s.primary_model, (modelCount.get(s.primary_model) ?? 0) + 1);
  }
  closeDb();
...
function apiKnowledgeSearch(q: string): unknown {
  if (!q.trim()) return { results: [] };
  const cfg = loadConfig();
  const results: Array<{ group: string; displayName: string; type: string; ref: string; preview: string }> = [];
  for (const key of Object.keys(cfg.groups)) {
    const trust = getGroupTrust(cfg, key);
    if (trust === 'deny') continue;
...
async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (path === '/api/decisions') {
      ...
      all.sort((a, b) => {
        const ka = (a.ts as string | undefined) ?? (a.date as string | undefined) ?? '';
        const kb = (b.ts as string | undefined) ?? (b.date as string | undefined) ?? '';
    ...
    if (projMatch) {
      ...
      const status = (data as { error?: string }).error ? 404 : 200;
    ...
    if (intentMatch && method === 'PUT') {
      ...
      const cfg = loadConfig();
      if (!cfg.groups[group]) return notFound(res);
      if (getGroupTrust(cfg, group) !== 'read-write') return bad(res, 'trust tier blocks writes');
      const body = await readBody(req);
      const content = body.content;
      if (typeof content !== 'string') return bad(res, 'content required');
    ...
    if (fyiAddMatch && method === 'POST') {
      ...
      if (typeof entry !== 'string' || !entry.trim()) return bad(res, 'entry required');
    ...
    if (trustMatch && method === 'PUT') {
      ...
      const b = await readBody(req);
      const t = b.trust;
      if (t !== 'read-write' && t !== 'read-only' && t !== 'deny') return bad(res, 'invalid trust tier');
      let cfg = loadConfig();
      if (!cfg.groups[group]) return notFound(res);
      cfg = setGroupTrust(cfg, group, t as TrustTier);
      saveConfig(cfg);
    ...
    if (path === '/api/llm/models' && method === 'GET') {
      if (!llmAvailable()) return json(res, 200, { provider: null, models: [], defaultModel: null });
    ...
    if (presetMatch && method === 'GET') {
      ...
      let prompt: string;
      if (which === 'audit' && docName) prompt = buildAuditModePrompt(group, docName);
      else prompt = buildArchitectModePrompt(group);
    ...
    if (tasksListMatch && method === 'POST') {
      ...
      if (!cfg.groups[group]) return notFound(res);
      const b = await readBody(req);
      if (typeof b.title !== 'string' || !b.title.trim()) return bad(res, 'title required');
      const t = createTask(group, {
        title: b.title.trim(),
        description: typeof b.description === 'string' ? b.description : undefined,
        priority: (['high', 'med', 'low'].includes(String(b.priority)) ? b.priority : 'med') as 'high' | 'med' | 'low',
    ...
    if (taskOneMatch && method === 'PATCH') {
      ...
      const b = await readBody(req);
      const t = updateTask(group, id, b as never);
    ...
    if (alignScanMatch && method === 'POST') {
      ...
      const cfg = loadConfig();
      if (!cfg.groups[group]) return notFound(res);
      if (!llmAvailable()) return bad(res, 'No LLM provider configured');
      try {
        const summary = await runFullScan(group);
        return json(res, 200, { summary, tasks: listTasks(group) });
      } catch (e) {
        return bad(res, `scan failed: ${(e as Error).message}`);
      }
    }
    ...
    if (alignExportMatch && method === 'GET') {
      ...
      lines.push(`---`);
      lines.push('');
      const byDoc = new Map<string, typeof tasks>();
      for (const t of tasks) {
        const k = t.docRef ?? '(unknown)';
        if (!byDoc.has(k)) byDoc.set(k, [] as never);
        byDoc.get(k)!.push(t);
      }
    ...
    if (taskPatchMatch && method === 'POST') {
      ...
      if (!doc) return bad(res, 'task has no resolvable docRef; set one before requesting a patch');
    ...
    if (chatOneMatch && method === 'PATCH') {
      ...
      const id = decodeURIComponent(chatOneMatch[2]!);
      const b = await readBody(req);
      const patch: { title?: string; model?: string; systemPrompt?: string; presetName?: string } = {};
      ...
      if (typeof b.systemPrompt === 'string') patch.systemPrompt = b.systemPrompt.slice(0, 8000);
      if (typeof b.presetName === 'string') patch.presetName = b.presetName.slice(0, 40);
    ...
    if (chatStreamMatch && method === 'POST') {
      ...
      if (!userMsg) return bad(res, 'message required');
      ...
      const attachedPaths: string[] = mentioned.map((m) => m.doc.absolutePath);
      let openDocBlock = '';
      if (openDocPath) {
      ...
      if (overrideModel !== session.model) touchChatSession(group, id, { model: overrideModel });
      ...
      let lastModel = overrideModel, lastProvider: string | undefined;
      const MAX_ITER = 10;
      let errored = false;
    ...
    if (docFileMatch && method === 'GET') {
      ...
      if (!wantedAbs) return bad(res, 'path required');
      ...
      try {
        ...
        return bad(res, `read failed: ${(e as Error).message}`);
      }
      return json(res, 200, {
        relPath: doc.relPath,
    ...
    if (pendingDecActMatch && method === 'POST') {
      ...
      const def = cfg.groups[group];
      if (!def) return notFound(res);
      if (action !== 'accept' && action !== 'reject') return bad(res, 'action must be accept|reject');
      ...
      if (action === 'accept') {
        ...
        if (llmAvailable()) {
          proposeDocPatch(group, def.projects, {
            ...
            impact: updated.impact,
            refs: updated.refs,
          }, def.docPaths).catch(() => { /* already logged */ });
        }
      }
      return json(res, 200, { ok: true });
    }
    ...
    if (decisionsMatch) {
      ...
      if (method === 'POST') {
        ...
        if (!hasStructured && !entry?.trim()) return bad(res, 'entry or title/why/impact required');
    ...
    log.error('server error', { err: (e as Error).message });
    autoReport({
...
export function startServer(port = 7777): void {
  const server = createServer((req, res) => {
    handle(req, res).catch((e) => {
      log.error('handler crash', { err: (e as Error).message });
      autoReport({
