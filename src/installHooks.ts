...
export interface InstallOptions {
  /** Override the path to the compiled CLI. Useful for dev mode. */
  cliPath?: string;
  dryRun?: boolean;
}
...
export interface InstallResult {
  ...
  commands: Array<{ event: string; command: string }>;
}
...
function buildCommand(cliPath: string, hookName: string): string {
  // Quote path for Windows paths that may contain spaces.
  return `node "${cliPath}" hook ${hookName}`;
}

function isOurHook(command: string, hookName: string): boolean {
...
export function installHooks(opts: InstallOptions = {}): InstallResult {
  ...
  if (!existsSync(cliPath)) {
    throw new HookInstallError(
      `CLI not found at ${cliPath}. Run "npm run build" first, or pass --cli-path.`,
      cliPath,
    );
  }
  ...
  const commands: Array<{ event: string; command: string }> = [];
  let added = 0;
  let updated = 0;
  ...
  for (const spec of HOOK_SPEC) {
    ...
    if (idx >= 0) {
      ...
      entry.hooks = entry.hooks.map((h) =>
        isOurHook(h.command, spec.hookName)
          ? ({ type: 'command', command } satisfies HookCommand)
          : h,
      );
      updated++;
    } else {
  ...
  if (!opts.dryRun) saveSettings(settings);
...
export function uninstallHooks(): { removed: number; backupPath: string | null } {
  const backupPath = backupSettings();
  const settings = loadSettings();
  if (!settings.hooks) return { removed: 0, backupPath };
  ...
  for (const [eventName, entries] of Object.entries(settings.hooks)) {
    ...
    for (const entry of entries) {
      const keptHooks = entry.hooks.filter(
        (h) =>
          !HOOK_SPEC.some((s) => isOurHook(h.command, s.hookName)),
      );
