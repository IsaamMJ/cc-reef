...
function openBrowser(url: string): void {
  try {
    ...
    log.warn('browser open failed', { err: (e as Error).message });
  }
}
...
export interface ReportBugResult {
  url: string;
  bodyLength: number;
  logLinesIncluded: number;
}

export function reportBug(opts: { noOpen?: boolean; title?: string } = {}): ReportBugResult {
...
export function previewBugReport(): string {
  const rawLog = readLastLines(REEF_LOG_FILE, MAX_LOG_LINES);
