...
export interface ModelGuide {
  id: string;
  family: string;          // friendly family name (e.g. "Grok 4")
  contextK?: number;       // approximate context window in K tokens
  tier: 'flagship' | 'fast' | 'small' | 'legacy' | 'unknown';
  bestFor: string[];       // 3-5 concrete use cases
  strengths: string[];
  weaknesses: string[];
  oneLiner: string;        // single sentence shown under the dropdown
}
...
export function guideFor(modelId: string): ModelGuide {
  ...
  if (prefixMatches[0]) return { ...prefixMatches[0], id: modelId };
  // Family fuzzy: anything containing "grok-4-fast" → grok-4-fast-reasoning.
  if (/grok-?4.*fast.*non/i.test(modelId)) return { ...GUIDES.find((g) => g.id === 'grok-4-fast-non-reasoning')!, id: modelId };
  if (/grok-?4.*fast/i.test(modelId)) return { ...GUIDES.find((g) => g.id === 'grok-4-fast-reasoning')!, id: modelId };
  if (/grok-?4/i.test(modelId)) return { ...GUIDES.find((g) => g.id === 'grok-4')!, id: modelId };
  if (/grok-?3.*mini/i.test(modelId)) return { ...GUIDES.find((g) => g.id === 'grok-3-mini')!, id: modelId };
  if (/grok-?3/i.test(modelId)) return { ...GUIDES.find((g) => g.id === 'grok-3')!, id: modelId };
  if (/gpt-?4o.*mini/i.test(modelId)) return { ...GUIDES.find((g) => g.id === 'gpt-4o-mini')!, id: modelId };
  if (/gpt-?4o/i.test(modelId)) return { ...GUIDES.find((g) => g.id === 'gpt-4o')!, id: modelId };
  return { ...FALLBACK, id: modelId };
}

export function recommendedFor(role: 'chat' | 'classify' | 'patch'): string[] {
  if (role === 'chat') return ['grok-4-fast-reasoning', 'grok-4', 'gpt-4o'];
  if (role === 'classify') return ['grok-3-mini', 'gpt-4o-mini', 'grok-4-fast-non-reasoning'];
  if (role === 'patch') return ['grok-4', 'grok-4-fast-reasoning', 'gpt-4o'];
  return [];
}
