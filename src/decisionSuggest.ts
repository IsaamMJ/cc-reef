...
/**
 * After a session ends, summarise what happened and ask the LLM to propose
 * 0–5 decisions. Each suggestion sits in `pending-decisions.json` until the
 * user accepts (→ promotes to a real decision in decisions.jsonl) or rejects.
 *
 * We never auto-log: the user always confirms. This keeps the decision log
 * trustworthy (no hallucinated entries).
 */
...
export function readPendingDecisions(group: string): PendingDecision[] {
  const p = pendingPath(group);
  if (!existsSync(p)) return [];
  try {
    const f = JSON.parse(readFileSync(p, 'utf8')) as PendingFile;
    return f.decisions ?? [];
  } catch {
    return [];
  }
...
function writePending(group: string, decisions: PendingDecision[]): void {
  const p = pendingPath(group);
  const dir = dirname(p);
...
export function setPendingStatus(group: string, id: string, status: 'accepted' | 'rejected'): PendingDecision | null {
...
export function clearProcessedPending(group: string): void {
...
async function summariseSession(transcriptPath: string): Promise<SessionSummary> {
  ...
  for await (const yielded of parseJsonl(transcriptPath)) {
    if (isParseFailure(yielded)) continue;
    const ev = yielded.event;
    const role = (ev as { type?: string }).type;
      ...
      if (Array.isArray(content)) {
        for (const block of content) {
          ...
          if (toolName === 'Bash' && typeof input.command === 'string') {
            ...
            if (cmd.length < 200) summary.bashCommands.push(cmd);
          }
        }
      }
    }
...
export interface SuggestedDecisionInput {
  group: string;
  sessionId: string;
  transcriptPath: string;
}

export async function suggestDecisionsForSession(opts: SuggestedDecisionInput): Promise<PendingDecision[]> {
  ...
  try {
    ...
    log.warn('decisionSuggest: summary failed', { err: (e as Error).message });
    return [];
  }

  // Heuristic: skip sessions with no edits AND few tools.
  if (summary.editedFiles.size === 0 && summary.totalAssistantTurns < 5) return [];
  ...
  let proposed: Array<{ title: string; why: string; impact: string; refs?: string[] }> = [];
  try {
    ...
    const obj = JSON.parse(res.text) as { decisions?: typeof proposed };
    proposed = obj.decisions ?? [];
  } catch (e) {
    log.warn('decisionSuggest: LLM call failed', { err: (e as Error).message });
    return [];
  }
  ...
  const newSuggestions: PendingDecision[] = proposed
