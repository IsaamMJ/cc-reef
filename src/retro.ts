...
function sinceIso(opts: RetroOptions): { iso: string; days: number } {
  if (opts.since) {
...
export function generateRetro(opts: RetroOptions = {}): string {
  ...
  // Per-group breakdown.
  const byGroup = new Map<string, { sessions: number; turns: number; tools: number }>();
  for (const s of sessions) {
