...
import { scanGroupRepoDocs } from './docs.js';
...
export interface AlignmentSummary {
  ...
  scannedAt: string;
  docsScanned: number;
  drifts: { high: number; med: number; low: number; total: number };
  errors: string[];
}

function summaryPath(group: string): string {
  return join(REEF_KNOWLEDGE, group.toLowerCase(), 'alignment.json');
}

export function readAlignmentSummary(group: string): AlignmentSummary | null {
  const p = summaryPath(group);
  if (!existsSync(p)) return null;
  try { return JSON.parse(readFileSync(p, 'utf8')) as AlignmentSummary; }
  catch { return null; }
}

function writeSummary(s: AlignmentSummary): void {
...
type ParseResult =
  | { ok: true; findings: DocFinding[] }
  | { ok: false; error: string };

/** Tolerantly extract a JSON object with `findings: [...]` from any LLM output. */
function parseFindingsJson(raw: string): ParseResult {
  let txt = raw.trim();
  ...
  if (end === -1) return { ok: false, error: 'unbalanced JSON' };
  try {
    const obj = JSON.parse(txt.slice(start, end)) as { findings?: DocFinding[] };
...
export async function runFullScan(group: string): Promise<AlignmentSummary> {
  const cfg = loadConfig();
  const def = cfg.groups[group];
  if (!def) throw new Error(`group not found: ${group}`);
  ...
  for (const d of docs) {
    try {
      ...
      try {
        ...
        errors.push(`${d.relPath}: LLM call failed — ${(e as Error).message}`);
        continue;
      }
      ...
      for (const f of findings) {
        if (!f.title) continue;
        const sev: 'high' | 'med' | 'low' = (['high', 'med', 'low'] as const).includes(f.severity) ? f.severity : 'med';
        // Don't duplicate: skip if there's an open task with same title + same docRef.
        const dup = listTasks(group).find((t) =>
          t.status !== 'done' && t.docRef === d.relPath && t.title.trim() === f.title.trim(),
        );
        if (dup) continue;
        createTask(group, {
        ...
        drifts[sev]++; drifts.total++;
      }
    } catch (e) {
      errors.push(`${d.relPath}: ${(e as Error).message}`);
      log.warn('alignment: doc scan failed', { doc: d.relPath, err: (e as Error).message });
    }
  }
...
function listTopFiles(group: string): string {
  const cfg = loadConfig();
  const def = cfg.groups[group];
  if (!def) return '';
  ...
  function resolveRoot(folder: string): string | null {
    ...
    function tryP(prefix: string, parts: string[]): string | null {
...
export async function applyTaskPatch(group: string, taskId: string): Promise<{ ok: true; doc: string; backup: string } | { ok: false; error: string }> {
  const t = getTask(group, taskId);
  if (!t) return { ok: false, error: 'task not found' };
  if (!t.docRef) return { ok: false, error: 'task has no docRef' };
  const ctx = buildGroupContext(group);
  const doc = resolveDocRef(ctx, t.docRef);
  if (!doc) return { ok: false, error: `doc not found: ${t.docRef}` };
  ...
  if (objStart === -1) return { ok: false, error: 'LLM did not return JSON' };
  ...
  if (!newBody) return { ok: false, error: 'empty replacement body' };
