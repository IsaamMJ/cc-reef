...
function extractText(line: string): { role: string; text: string } | null {
  try {
    ...
    if (typeof c === 'string') text = c;
      ...
      for (const item of c) {
        if (typeof item === 'object' && item !== null) {
          const it = item as { type?: string; text?: string; content?: string };
          if (it.type === 'text' && typeof it.text === 'string') text += it.text + '\n';
          else if (it.type === 'tool_use' && (it as { name?: string }).name) text += `[tool: ${(it as { name?: string }).name}]\n`;
        }
      }
    }
    text = text.trim();
...
function buildPrompt(s: SessionForSummary, firstUser: string, lastAssistant: string): string {
  return [
    `Project: ${s.project}`,
...
export async function summarizeSession(sessionId: string, force = false): Promise<string | null> {
  if (!nimAvailable()) throw new Error('NIM_API_KEY not set');
  const db = getDb();
  const row = db.prepare(
    `SELECT session_id, source_file, project, turn_count, tool_call_count, primary_model, summary
     FROM sessions WHERE session_id = ?`,
  ).get(sessionId) as (SessionForSummary & { summary?: string | null }) | undefined;
  if (!row) return null;
  if (row.summary && !force) return row.summary;
  ...
  for (const line of raw) {
    ...
    if (!p) continue;
    if (!firstUser && (p.role === 'user' || p.role === 'human')) firstUser = p.text;
    if (p.role === 'assistant') lastAssistant = p.text;
  }
...
export async function summarizeRecent(limit = 20, force = false): Promise<{ done: number; failed: number; skipped: number }> {
  if (!nimAvailable()) throw new Error('NIM_API_KEY not set');
  const db = getDb();
  ...
  for (const r of rows) {
    try {
      ...
      log.error('summarize failed', { sid: r.session_id, err: (e as Error).message });
      failed++;
    }
  }
...
export async function aiOverviewLine(days: number): Promise<string | null> {
  if (!nimAvailable()) return null;
  const db = getDb();
  const since = new Date(Date.now() - days * 86400000).toISOString();
  const rows = db.prepare(
    `SELECT s.summary, s.project, s.ended_at FROM sessions s
     WHERE s.ended_at IS NOT NULL AND s.ended_at >= ? AND s.summary IS NOT NULL AND s.summary != ""
     ORDER BY s.ended_at DESC LIMIT 30`,
  ).all(since) as Array<{ summary: string; project: string; ended_at: string }>;
  ...
  try {
    ...
    log.error('ai overview line failed', { err: (e as Error).message });
    return null;
  }
}
