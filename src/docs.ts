...
import { classifyHeuristic, TIER_FOR_TYPE, type DocType } from './docTaxonomy.js';
...
export interface DocFile {
  ...
  preview: string;
  mtime: number;
  // Phase-1 taxonomy fields. `inferred` = no frontmatter, classifier guessed.
  type: DocType;
  tier: number;
  status?: string;
  owners?: string[];
...
function isValuable(absolutePath: string, rootPath: string): boolean {
  ...
  if (VALUABLE_NAMES.has(name)) return true;
  const parentFolder = parts.length >= 2 ? (parts[parts.length - 2] ?? '').toLowerCase() : '';
  if (VALUABLE_FOLDERS.has(parentFolder)) return true;
  return false;
}

function scanDir(dir: string, rootPath: string, depth: number, results: string[]): void {
  if (depth > 4) return;
  let entries: string[];
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      ...
      if (isValuable(full, rootPath)) results.push(full);
    }
  }
}

function buildDocFile(abs: string, rel: string, folderName: string): DocFile {
  const content = (() => { try { return readFileSync(abs, 'utf8'); } catch { return ''; } })();
  ...
  const mtime = (() => { try { return statSync(abs).mtimeMs; } catch { return 0; } })();

  // Classification: explicit frontmatter wins; else heuristic.
  let type: DocType;
  let tier: number;
  let inferred: boolean;
  const fmType = typeof fm.data.type === 'string' ? (fm.data.type as DocType) : undefined;
  ...
  const owners = Array.isArray(fm.data.owners) ? (fm.data.owners as unknown[]).map(String) : undefined;
...
function extractSections(body: string): Array<{ id: string; title: string; level: number }> {
...
export function scanRepoDocs(diskPath: string, folderName: string): DocFile[] {
...
export function resolveProjectPath(folderName: string): string | null {
  ...
  function tryPaths(prefix: string, parts: string[]): string | null {
    if (parts.length === 0) return existsSync(prefix) ? prefix : null;
...
export function scanGroupRepoDocs(projects: string[], extraDocPaths: string[] = []): DocFile[] {
  const all: DocFile[] = [];
...
function scanDirAll(dir: string, depth: number, out: string[]): void {
  if (depth > 6) return;
  let entries: string[];
  try { entries = readdirSync(dir); } catch { return; }
  for (const entry of entries) {
    ...
    let stat;
    try { stat = statSync(full); } catch { continue; }
    if (stat.isDirectory()) scanDirAll(full, depth + 1, out);
    else if (stat.isFile() && extname(entry).toLowerCase() === '.md') out.push(full);
  }
}

export function getDecisionsFilePath(group: string): string {
  return join(REEF_KNOWLEDGE, group.toLowerCase(), 'decisions.md');
}

export function getDecisionsJsonlPath(group: string): string {
...
function buildBody(d: DecisionInput): string {
  if (d.body && d.body.trim()) return d.body.trim();
  const parts: string[] = [];
  if (d.title) parts.push(`**${d.title.trim()}**`);
  if (d.why) parts.push(`*Why:* ${d.why.trim()}`);
  if (d.impact) parts.push(`*Impact:* ${d.impact.trim()}`);
  if (d.refs && d.refs.length) parts.push(`*Refs:* ${d.refs.join(', ')}`);
  return parts.join('\n\n');
}

function readJsonlDecisions(group: string): DecisionEntry[] {
  ...
  raw.split('\n').forEach((line, i) => {
    ...
    if (!t) return;
    try {
      const o = JSON.parse(t) as Partial<DecisionEntry> & { ts?: string };
      const ts = o.ts ?? '';
      ...
      out.push({
        ...
        refs: o.refs,
        project: o.project,
        body: o.body ?? buildBody(o as DecisionInput),
        index: i,
        source: 'jsonl',
      });
    } catch {
...
function readMarkdownDecisions(group: string): DecisionEntry[] {
...
export function readDecisions(group: string): DecisionEntry[] {
...
export function appendDecision(group: string, entry: string | DecisionInput): void {
  const path = getDecisionsJsonlPath(group);
