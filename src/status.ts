...
export interface ReefStatus {
  hooksInstalled: { sessionStart: boolean; bashNudge: boolean; postSession: boolean };
  settingsPath: string;
  dbExists: boolean;
  dbSizeBytes: number;
  sessions: number;
...
function detectHooks(): ReefStatus['hooksInstalled'] {
  const out = { sessionStart: false, bashNudge: false, postSession: false };
  if (!existsSync(CLAUDE_SETTINGS)) return out;
  try {
...
export function getStatus(): ReefStatus {
  ...
  if (dbExists) {
    ...
    try {
      ...
      toolCalls = (db.prepare('SELECT COUNT(*) c FROM tool_calls').get() as { c: number }).c;
      const row = db.prepare('SELECT MAX(scanned_at) m FROM sessions').get() as { m: string | null };
      lastScannedAt = row.m;
    } finally {
      closeDb();
    }
  ...
  for (const g of Object.values(cfg.groups)) {
    for (const p of g.projects) assigned.add(p);
  }
...
export function printStatus(s: ReefStatus): void {
